	...
	public List<Intersection> path (Intersection start, Intersection end){
		...
		while (!workingPath.isEmpty()){
			...
			for (AStarNode node : cur.getNeighbours()){
				...
				Segment pair=null;
				List<Segment> curSegs=cur.getSegments();
				...
				double totalCost=cur.costFromStart +pair.getLength();
				if(!workingPath.contains(node))workingPath.add(node);
				else if (totalCost>=node.getCostFromStart()){
					continue;
				}
				node.setParent(cur);
				node.setCostFromStart(totalCost);
